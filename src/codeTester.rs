...
fn main() {
    ...
    println!("{:?}",compact_decode(vec![0,97]));
            ...
            Node::Flag((encoded_prefix, value)) => {
                ...
                if first_nibbel == 2 || first_nibbel == 3 {
                    ...
                    if current_node_nibbel_decoded[0] == key[0]{
                        ...
                        let test = compact_encode(ext_node_nibbel);
                        println!("Ext node encoded  = {:?}", test);

                        let exstention_node = Node::Flag((test, hash_node(&branch_node)));
                        let exstention_node_hash = hash_node(&exstention_node);
                        self.db.insert(hash_node(&branch_node),branch_node);
                        ...
                        return_value_hash = exstention_node_hash;
                    } else {
                        let mut branch_array: [String; 17] = Default::default();
        ...
        while !done {
            ...
            match node {
                Node::Branch(branch) => {
                    ...
                    if key_prefix.len() == 0 {
                        println!("Found the value in branch");
                ...
                Node::Flag((encoded_prefix, value)) => {
                    ...
                    println!(" Mathc with: {:?}", node_prefix_decoded);
                    ...
                    if first_nibbel == 3 || first_nibbel == 4 {
                        ...
                        println!("Ext node nibbel: {:?}", node_prefix_decoded);
                        println!("Ext node nibbel: {:?}",key_prefix );
